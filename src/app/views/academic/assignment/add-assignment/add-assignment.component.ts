...
export class AddAssignmentComponent implements OnInit {
  ...
  addAssignment() {
    ...
    this.assigmentService.uploadFile(uploadFile).subscribe((file: any) => {
      this.createAssigmentForm.controls["file"].setValue(file.filename);

      this.assigmentService
        .addAssignment(this.createAssigmentForm.value)
        .subscribe(
          assignment => {
            this.toastr.success("Assignment has been added successfully ");

            this.router.navigate(["academic/assignment/assignment-list"]);
          },
          error => {
            this.toastr.error("Error while adding Assignment", "", {
              timeOut: 3000
            });
          }
        );
